...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        try {
            ...
            if(serverQueue.songs.length === 1)queue.setDescription(`No songs. To play next add songs by \`${settings.prefix}play <song_name> | ${settings.prefix}play <song_link>\``)
            message.channel.send(queue)
        } catch {
            sendError("There's nothing playing", message.channel);
        }
    }
}